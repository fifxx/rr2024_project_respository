...
# Subset data to remove rows with missing values in logem4 and avexpr
...
ggplot(df1_subset2, aes(x = logem4, y = avexpr)) +
  ...
  geom_text(aes(x = logem4, y = avexpr, label = labels), hjust = 0, vjust = 0) +
...
df4$predicted_avexpr <- predict(model_fs, newdata = df4)  # Use predict from the model
...
# Correctly estimate a 2SLS regression
